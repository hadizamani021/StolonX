...
| masterEligibilitySelector        | Selector used by the sentinel to decide which keepers are eligible to become the master. Summary: `--can-be-master=false` overrides the selector; only `matchLabels` with exact key/value matching; patchable live via `stolonctl update`. Full behaviour and examples: [Master eligibility selector](master_eligibility_selector.md). | no                        | object                   | null                                                                                                                                |
...
| pgRewindRetry                    | when `usePgrewind` is true, controls how many times the keeper retries `pg_rewind` on resync before falling back to `pg_basebackup`. Omitted means **one attempt only** (no retries). See [pg_rewind retries](pg_rewind.md#retries).                                                                                                                                                                                                                                                                            | no                        | object            | `maxAttempts` 1, `interval` 0s, `backoffMultiplier` 1 (single attempt, same as omitting the object)                               |
...
#### PgRewindRetryPolicy (object `pgRewindRetry`)

| Name                | Description                                                                                                                                                                                                 | Required | Type              | Default |
|---------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|-------------------|---------|
| maxAttempts         | Total number of `pg_rewind` runs attempted for a single resync before falling back to `pg_basebackup` (includes the first run).                                                                              | no       | uint32            | 1       |
| interval            | Base delay before the second and subsequent attempts after a failure. Before attempt *k* (*k* ≥ 2), the keeper sleeps `interval * backoffMultiplier^(k-2)`.                                                  | no       | string (duration) | 0s      |
| backoffMultiplier | Multiplier for the delay after each failed attempt; must be ≥ 1. `1` means the same `interval` between every retry.                                                                                          | no       | float64           | 1       |
