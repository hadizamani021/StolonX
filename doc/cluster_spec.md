...
| masterEligibilitySelector        | Selector used by the sentinel to decide which keepers are eligible to become the master. If `--can-be-master` is set to `false` the selector is ignored. Only `matchLabels` is supported. Matching is exact on both key and value. This can be updated dynamically with `stolonctl update` without restarting keepers. | no                        | object                   | null                                                                                                                                |                                                                                                                                  |
